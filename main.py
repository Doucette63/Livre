...
# Combien de recherches on interroge au maximum par cycle, pour limiter les
# rafales de requêtes rapprochées (repéré par l'anti-bot au-delà de ~3).
MAX_URLS_PER_CYCLE = 3
...
# ----------------------
# 4ter. ROTATION DES RECHERCHES PAR LOTS
# ----------------------
_url_cursor = 0

def next_url_batch():
    """Renvoie jusqu'à MAX_URLS_PER_CYCLE URLs, en tournant sur la liste
    complète d'un cycle à l'autre. Avec 3 URLs ou moins, ça revient à tout
    checker à chaque fois comme avant."""
    global _url_cursor
    n = len(VINTED_URLS)
    if n <= MAX_URLS_PER_CYCLE:
        return list(VINTED_URLS)
    batch = [VINTED_URLS[(_url_cursor + i) % n] for i in range(MAX_URLS_PER_CYCLE)]
    _url_cursor = (_url_cursor + MAX_URLS_PER_CYCLE) % n
    return batch
...
def check_vinted():
    ...
    batch = next_url_batch()
    logger.info(f"📋 Lot de ce cycle : {len(batch)}/{len(VINTED_URLS)} recherche(s)")

    for i, url in enumerate(batch):
        if i > 0:
            spacing = random.uniform(4, 10)
            logger.info(f"⏳ Pause de {spacing:.1f}s avant la recherche suivante (anti-bot)")
            time.sleep(spacing)
